...
def construct_graph(ac_num, max_dis, memory):
	...
	# whole_graph[1, :, 0:1] = doc_graph.T
	#
	# whole_graph[2, 1:ac_num+1, 1:ac_num+1] = actc_graph
	#
	# whole_graph[3, ac_num + 1:ac_num + 1 + pair_num, ac_num + 1:ac_num + 1 + pair_num] = ari_graph
	#
	# whole_graph[4, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2:, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2] = artc_graph
	#
	# whole_graph[5, 1:ac_num + 1, ac_num + 1:ac_num + 1 + pair_num] = ac2ari_graph
	# whole_graph[6, ac_num + 1:ac_num + 1 + pair_num, 1:ac_num + 1] = ac2ari_graph.T
	#
	# whole_graph[7, 1:ac_num + 1:, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2] = ac2artc_graph
	# whole_graph[8, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2, 1:ac_num + 1:] = ac2artc_graph.T
	#
	# whole_graph[9, ac_num + 1:ac_num + 1 + pair_num, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2] = ari2artc_graph
	# whole_graph[10, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2, ac_num + 1:ac_num + 1 + pair_num] = ari2artc_graph.T
...
def construct_graph1_2(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 2)).tolist(), np.ones((1, 1)).tolist(), [], [], [], [], [], np.ones((3, 2, 2)).tolist()
	...
	# TODO whether considering the interaction between ari and artc or not
	ari2artc_graph = np.ones((pair_num, pair_num)) # whether needed only a self-loop graph ?
...
def construct_graph1_3(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 2)).tolist(), np.ones((1, 1)).tolist(), [], [], [], [], [], np.ones((3, 2, 2)).tolist()
	...
	# TODO whether considering the interaction between ari and artc or not
	ari2artc_graph = np.zeros((pair_num, pair_num)) # 1 only the pair has the same ac_id
	...
	for i, pair1 in enumerate(list(pair2sequence[ac_num])):
		for j, pair2 in enumerate(list(pair2sequence[ac_num])):
			if pair1[0] == pair2[0] or pair1[1] == pair2[0] or pair1[0] == pair2[1] or pair1[1] == pair2[1]:
				ari2artc_graph[i, j] = 1
...
def construct_graph2(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 2)).tolist(), np.ones((1, 1)).tolist(), [], [], [], [], [], np.ones((3, 2, 2)).tolist()
	...
	doc_graph = np.ones((1, ac_num + pair_num * 2))
	actc_graph = 1- np.eye(ac_num) # TODO consider the structure of paragraph
	ari_graph = 1- np.eye(pair_num) # whether needed full connected graph or only a self-loop graph ?
	artc_graph = 1 - np.eye(pair_num) # whether needed full connected graph or only a self-loop graph ?
	ac2ari_graph = np.zeros((ac_num, pair_num))
	ac2artc_graph = np.zeros((ac_num, pair_num))
	...
	whole_graph[0, 0:1, 1:] = doc_graph
	whole_graph[0, 1:, 0:1] = doc_graph.T
...
def construct_graph3(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 2)).tolist(), np.ones((1, 1)).tolist(), [], [], np.ones((2, 2, 2)).tolist()
	...
	whole_graph = np.zeros((4, 1 + ac_num + pair_num * 2, 1 + ac_num + pair_num * 2))
	...
	whole_graph[2, 1:ac_num + 1, ac_num + 1:ac_num + 1 + pair_num] = ac2ari_graph
	whole_graph[2, ac_num + 1:ac_num + 1 + pair_num, 1:ac_num + 1] = ac2ari_graph.T

	whole_graph[3, 1:ac_num + 1:, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2] = ac2artc_graph
	whole_graph[3, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2, 1:ac_num + 1:] = ac2artc_graph.T


	memory[ac_num] = doc_graph.tolist(), actc_graph.tolist(), ac2ari_graph.tolist(), ac2artc_graph.tolist(), whole_graph.tolist()

	return doc_graph.tolist(), actc_graph.tolist(), ac2ari_graph.tolist(), ac2artc_graph.tolist(), whole_graph.tolist()


def construct_graph4(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 2)).tolist(), np.ones((1, 1)).tolist(), [], [], [], np.ones((2, 2, 2)).tolist()
	...
	whole_graph = np.zeros((5, 1 + ac_num + pair_num * 2, 1 + ac_num + pair_num * 2))
	...
	whole_graph[4, ac_num + 1:ac_num + 1 + pair_num, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2] = ari2artc_graph
	whole_graph[4, ac_num + 1 + pair_num:ac_num + 1 + pair_num * 2, ac_num + 1:ac_num + 1 + pair_num] = ari2artc_graph.T

	memory[ac_num] = doc_graph.tolist(), actc_graph.tolist(), ac2ari_graph.tolist(), ac2artc_graph.tolist(), \
	                 ari2artc_graph.tolist(), whole_graph.tolist()

	return doc_graph.tolist(), actc_graph.tolist(), ac2ari_graph.tolist(), ac2artc_graph.tolist(), \
	       ari2artc_graph.tolist(), whole_graph.tolist()


def construct_graph5(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 2)).tolist(), np.ones((1, 1)).tolist(), [], [], [], [], np.ones((2, 2, 2)).tolist()
	...
	whole_graph = np.zeros((6, 1 + ac_num + pair_num * 2, 1 + ac_num + pair_num * 2))
	...
	memory[ac_num] = doc_graph.tolist(), actc_graph.tolist(), ari_graph.tolist(), artc_graph.tolist(), \
	                 ac2ari_graph.tolist(), ac2artc_graph.tolist(), whole_graph.tolist()
...
def construct_graph6_2(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
...
def construct_graph6_3(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	# TODO whether considering the interaction between ari and artc or not
	ari2artc_graph = np.zeros((pair_num, pair_num)) # whether needed only a self-loop graph ?
...
def construct_graph7(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	doc_graph = np.zeros((1, 1 + ac_num + pair_num * 2 + 3))
	doc_graph[0, 1:2] = 1
	doc_graph[0, ac_num+2:ac_num+3] = 1
	doc_graph[0, ac_num+pair_num+3:ac_num+pair_num+4] = 1
...
def construct_graph8(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	doc_graph = np.ones((1, 1 + ac_num + pair_num * 2 + 3))
	t_ac2ac_graph = np.zeros((1, 1 + ac_num + pair_num * 2 + 3))
	t_ac2ac_graph[0, 1 + ac_num] = 1
	t_ac2ac_graph[0, ac_num + 2:ac_num + 3] = 1
	t_ac2ac_graph[0, ac_num + 3 + pair_num:ac_num + 4 + pair_num] = 1

	actc_graph = np.ones((ac_num, ac_num)) # TODO consider the structure of paragraph
	t_ari2ari_graph = np.zeros((1, 1 + ac_num + pair_num * 2 + 3))
	t_ari2ari_graph[0, ac_num + 2:ac_num + 3 + pair_num] = 1
	t_ac2ac_graph[0, 1:2] = 1
	t_ac2ac_graph[0, ac_num + 3 + pair_num:ac_num + 4 + pair_num] = 1

	ari_graph = np.ones((pair_num, pair_num)) # whether needed full connected graph or only a self-loop graph ?
	t_artc2artc_graph = np.zeros((1, 1 + ac_num + pair_num * 2 + 3))
	t_artc2artc_graph[0, ac_num + 3 + pair_num:] = 1
	t_ac2ac_graph[0, 1:2] = 1
	t_ac2ac_graph[0, ac_num + 2:ac_num + 3] = 1
	...
	whole_graph[1, 1:2, :] = t_ac2ac_graph
	whole_graph[1, :, 1:2] = t_ac2ac_graph.T

	whole_graph[2, 2:ac_num + 2, 2:ac_num + 2] = actc_graph

	whole_graph[3, ac_num + 2:ac_num + 3, :] = t_ari2ari_graph
	whole_graph[3, :, ac_num + 2:ac_num + 3] = t_ari2ari_graph.T

	whole_graph[4, ac_num + 3:ac_num + 3 + pair_num, ac_num + 3:ac_num + 3 + pair_num] = ari_graph

	whole_graph[5, ac_num + 3 + pair_num:ac_num + 4 + pair_num, :] = t_artc2artc_graph
	whole_graph[5, :, ac_num + 3 + pair_num:ac_num + 4 + pair_num] = t_artc2artc_graph.T
...
def construct_graph9(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	doc_graph = np.ones((1, 1 + ac_num + pair_num * 2 + 3))
	t_ac2ac_graph = np.ones((1, 1 + ac_num + pair_num * 2 + 3))
	actc_graph = np.ones((ac_num, ac_num)) # TODO consider the structure of paragraph
	t_ari2ari_graph = np.ones((1, 1 + ac_num + pair_num * 2 + 3))
	ari_graph = np.ones((pair_num, pair_num)) # whether needed full connected graph or only a self-loop graph ?
	t_artc2artc_graph = np.ones((1, 1 + ac_num + pair_num * 2 + 3))
	artc_graph = np.ones((pair_num, pair_num)) # whether needed full connected graph or only a self-loop graph ?
	ac2ari_graph = np.zeros((ac_num, pair_num))
	ac2artc_graph = np.zeros((ac_num, pair_num))
...
def construct_graph10(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	whole_graph = np.zeros((7, 1 + ac_num + pair_num * 2 + 3, 1 + ac_num + pair_num * 2 + 3))
	...
	whole_graph[4, ac_num + 3 + pair_num:ac_num + 4 + pair_num, ac_num + 3 + pair_num:] = t_artc2artc_graph
	whole_graph[4, ac_num + 3 + pair_num:, ac_num + 3 + pair_num:ac_num + 4 + pair_num] = t_artc2artc_graph.T

	whole_graph[5, 2:ac_num + 2, ac_num + 3:ac_num + 3 + pair_num] = ac2ari_graph
	whole_graph[5, ac_num + 3:ac_num + 3 + pair_num, 2:ac_num + 2] = ac2ari_graph.T

	whole_graph[6, 2:ac_num + 2, ac_num + 4 + pair_num:ac_num + 4 + pair_num * 2] = ac2artc_graph
	whole_graph[6, ac_num + 4 + pair_num:ac_num + 4 + pair_num * 2, 2:ac_num + 2] = ac2artc_graph.T

	memory[ac_num] = doc_graph.tolist(), t_ac2ac_graph.tolist(), actc_graph.tolist(), t_ari2ari_graph.tolist(), \
	                 t_artc2artc_graph.tolist(), ac2ari_graph.tolist(), \
	                 ac2artc_graph.tolist(), whole_graph.tolist()
...
def construct_graph11(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	whole_graph = np.zeros((8, 1 + ac_num + pair_num * 2 + 3, 1 + ac_num + pair_num * 2 + 3))
	...
	whole_graph[7, ac_num + 3:ac_num + 3 + pair_num, ac_num + 4 + pair_num:ac_num + 4 + pair_num * 2] = ari2artc_graph
	whole_graph[7, ac_num + 4 + pair_num:ac_num + 4 + pair_num * 2, ac_num + 3:ac_num + 3 + pair_num] = ari2artc_graph.T
...
def construct_graph12(ac_num, max_dis, memory):
	if ac_num == 0:
		return [], [], [], [], [], [], [], [], [], []
	if ac_num == 1:
		return np.ones((1, 3)).tolist(), np.ones((1, 3)).tolist(), [], [], [], [], [], [], [], np.ones((3, 3, 3)).tolist()
	...
	whole_graph = np.zeros((9, 1 + ac_num + pair_num * 2 + 3, 1 + ac_num + pair_num * 2 + 3))
